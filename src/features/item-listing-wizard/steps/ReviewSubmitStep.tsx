...
import { DeclarationsBlock } from "../components/DeclarationsBlock";
import { ReviewRow, ReviewSection } from "../components/ReviewSection";
import { needsDimensions, needsWorkingStatus, uploadedUrls, type WizardModel, type WizardStep } from "../wizardModel";
    ...
    <div className="space-y-3">
      <ReviewSection title="Photos" onEdit={() => onJump("photos")}>
        <div className="flex flex-wrap gap-1.5">
          {urls.length === 0 && <p className="text-[11px] text-stone-400">No photos uploaded yet</p>}
          {urls.map((url, i) => (
            <div key={url} className="relative h-14 w-14 overflow-hidden rounded-lg border border-stone-200 dark:border-zinc-700">
              ...
              {i === 0 && (
                <span className="absolute inset-x-0 bottom-0 bg-[var(--ck-role-accent)] text-center text-[8px] font-black uppercase text-white">
                  Main
                </span>
              )}
            </div>
      ...
      <ReviewSection title="Item basics" onEdit={() => onJump("basics")}>
        ...
        <ReviewRow label="Brand" value={model.brand} />
        <ReviewRow label="Model" value={model.model} />
      </ReviewSection>
      ...
      <ReviewSection title="Condition & details" onEdit={() => onJump("condition")}>
        ...
        {needsWorkingStatus(model.category) && <ReviewRow label="Working status" value={model.workingStatus} />}
        ...
        {needsDimensions(model.category) && <ReviewRow label="Dimensions" value={model.dimensions} />}
        {needsDimensions(model.category) && <ReviewRow label="Weight" value={model.approximateWeight} />}
      ...
      <section className="rounded-xl border border-stone-200 bg-stone-50 p-3 dark:border-zinc-800 dark:bg-zinc-900">
        <h3 className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-stone-500 dark:text-stone-400">
          <Info className="h-3.5 w-3.5" aria-hidden /> What happens next
        </h3>
        <ol className="mt-1.5 list-decimal space-y-1 pl-4 text-[11px] text-stone-600 dark:text-stone-300">
