...
/** Derives notifications from current state of the user's matches/requests/listings. */
    ...
    requests.forEach(r => {
      if (r.status === "POTENTIAL_MATCH_FOUND" || r.status === "VERIFIED_PRIVATE_MATCHING") {
        notifs.push({
          id: `match-req-${r.id}`,
          title: "Match found!",
          body: `A donor was matched to "${r.title}"`,
          type: "match",
      ...
      if (r.status === "PUBLIC_REQUEST") {
        notifs.push({
          id: `approved-req-${r.id}`,
          title: "Request approved",
          body: `"${r.title}" is now live on the platform`,
          type: "approved",
          link: "/dashboard#my-requests",
          timestamp: Date.now(),
        });
    ...
    matches.forEach(m => {
      if (m.status === "TRANSPORT_DISCUSSION") {
        notifs.push({
          ...
          body: `Donor contact shared for "${m.requestTitle ?? "your request"}"`,
          type: "match",
          link: "/dashboard",
          timestamp: Date.now(),
        });
      ...
      if (m.status === "FULFILLED") {
        notifs.push({
          id: `fulfilled-${m.id}`,
          title: "Item received! 🎉",
          body: `"${m.requestTitle ?? "Your request"}" has been fulfilled`,
          type: "fulfilled",
          link: "/dashboard",
          timestamp: Date.now(),
        });
    ...
    matches.forEach(m => {
      if (m.status === "TRANSPORT_DISCUSSION") {
        notifs.push({
          id: `donor-match-${m.id}`,
          title: "New match!",
          body: `Your item matched with "${m.requestTitle ?? "a request"}"`,
          type: "match",
          link: "/dashboard",
          timestamp: Date.now(),
        });
      ...
      if (m.status === "PENDING_APPROVAL") {
        notifs.push({
          ...
          body: `Your donation to "${m.requestTitle ?? "a request"}" is being reviewed`,
          type: "info",
          link: "/dashboard",
          timestamp: Date.now(),
        });
      ...
      if (m.status === "FULFILLED") {
        notifs.push({
          ...
          body: `"${m.requestTitle ?? "Your donation"}" was delivered successfully`,
          type: "fulfilled",
          link: "/dashboard",
          timestamp: Date.now(),
        });
...
export function useNotifications() {
  ...
  const refresh = useCallback(async () => {
    if (!user) return;
    const notifs = await deriveNotifications(user.role);
    const seen   = loadSeen();
    const count  = notifs.filter(n => !seen.has(n.id)).length;
