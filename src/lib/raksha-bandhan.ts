 ...
 *
 * Set NEXT_PUBLIC_RAKSHA_BANDHAN to `on` to force the campaign on, `off` to
 * turn it off immediately, or leave it unset to use the date window.
 *
 * <p><b>The window runs 27 August to the end of 1 September 2026 (IST)</b>, not
 * the single day this originally carried. Two Raksha Bandhan treatments were
 * built independently and merged: this one was scoped to the festival day
 * itself, the other to a week-long run ending Tuesday. The week won, because
 * that is the run that was asked for — so the strip, the nav dressing, the
 * wordmark, the hero card and the homepage section all share it.
 *
 * <p><b>The full-screen intro is the one exception and keeps its own gate.</b>
 * See {@link isRakshaBandhanIntroDay}: it runs only on 28 August. An intro that
 * took over the screen every day for a week would stop being an arrival and
 * become an obstacle.
 *
 * <p>Expressed in UTC so it lines up exactly with midnight-to-midnight IST —
 * IST is UTC+5:30, so a window opening at midnight IST opens at 18:30Z the day
 * before. Both ends arrive on their own, with no deploy needed at either.
 */
...
// ─── The longest-unclaimed need ──────────────────────────────────────────────
//
// Moved to `./needTiming` and re-exported here.
//
// Nothing about "how long has this need waited" is seasonal, and the landing
// page's narrative is now built on it — so leaving the implementations in this
// campaign module would have meant retiring the campaign took them down too.
// Re-exported rather than relocated at every call site, so this module's
// existing consumers and tests are untouched.
export { daysWaiting, waitingLabel, longestWaiting } from "./needTiming";
