...
export default function BlogReadingPage({ params }: PageProps) {
  ...
  return (
    <div id="page-body" className={`${fontMode} ${boldMode ? "bold-mode-active" : ""} min-h-screen transition-colors duration-300`}>
      {/* TopAppBar */}
      <header className="fixed top-0 w-full z-50 h-20 transition-all duration-300 border-b border-outline-variant/60 bg-surface-cream/95 backdrop-blur-md">
        <nav className="flex justify-between items-center h-full px-margin-mobile md:px-lg max-w-container-max mx-auto">
          <div className="flex items-center gap-base md:gap-lg">
            <Link
              href="/"
              className="font-display-lg-mobile text-[28px] md:text-[32px] font-bold text-primary transition-colors hover:opacity-90"
            >
              ImpactStory
            </Link>
          </div>
          <div className="flex items-center gap-sm">
            <Link
              href="/"
              className="flex items-center gap-1.5 text-sm font-label-md text-on-surface-variant hover:text-rust-action transition-colors group px-3 py-1.5 rounded-full hover:bg-surface-container-low"
            >
      ...
      {/* Footer */}
      <footer className="w-full py-xl px-gutter flex flex-col md:flex-row justify-between items-center gap-md bg-surface-container-low dark:bg-inverse-surface">
        <div className="font-headline-md text-headline-md text-primary dark:text-inverse-primary">
          Kind Earth Journal
        </div>
        <nav className="flex flex-wrap justify-center gap-6">
          <Link
            className="font-plusJakartaSans text-label-sm text-on-surface-variant dark:text-surface-variant hover:text-rust-action transition-colors duration-200 cursor-pointer"
            href="#"
          >
            Privacy Policy
          </Link>
        ...
        <div className="font-plusJakartaSans text-label-sm text-secondary dark:text-secondary-fixed">
        </div>
      </footer>
    </div>
  );
