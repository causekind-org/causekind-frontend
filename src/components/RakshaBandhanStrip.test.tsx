...
describe("isRakshaBandhanCampaignActive", () => {
  it("covers exactly one IST day", () => {
    // 18:30Z on the 27th is midnight IST on the 28th — the first instant.
    expect(isRakshaBandhanCampaignActive(new Date("2026-08-27T18:30:00.000Z"), undefined)).toBe(true);
    expect(isRakshaBandhanCampaignActive(DURING, undefined)).toBe(true);
    // One millisecond before IST midnight on the 29th is still in.
    expect(isRakshaBandhanCampaignActive(new Date("2026-08-28T18:29:59.999Z"), undefined)).toBe(true);
  });

  it("is closed on either side of that day", () => {
    expect(isRakshaBandhanCampaignActive(new Date("2026-08-27T18:29:59.999Z"), undefined)).toBe(false);
    expect(isRakshaBandhanCampaignActive(new Date("2026-08-28T18:30:00.000Z"), undefined)).toBe(false);
