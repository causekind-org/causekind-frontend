...
/** Donation/campaign analytics — previously the standalone /admin/analytics page,
 * now a tab inside the single admin dashboard. */
export function AnalyticsPanel() {
  ...
  if (loadError) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 text-center py-20 px-4">
        <XCircle className="size-8 text-red-400" />
        <p className="text-sm text-stone-600">Couldn't load analytics data — the server may be having an issue. Try refreshing.</p>
      </div>
    );
  }
          ...
          <CardContent>
            {topCampaigns.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={topCampaigns} layout="vertical" margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                  ...
                  <Bar dataKey="amount" radius={[0, 4, 4, 0]}>
                    {topCampaigns.map((_, i) => (
                      <Cell key={i} fill={i === 0 ? "#b04a15" : i === 1 ? "#e07b3a" : "#f0b97a"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="overflow-x-auto rounded-lg border">
              <table className="w-full text-sm">
                <thead className="border-b bg-muted/50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("colDate")}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("colDonor")}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t("colCampaign")}</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground">{t("colAmount")}</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-muted-foreground">{t("colStatus")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {donations.slice(0, 20).map((d) => (
                    <tr key={d.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="max-w-[200px] truncate px-4 py-3 text-sm">{d.campaignTitle}</td>
                      <td className="whitespace-nowrap px-4 py-3 text-right font-semibold">{formatINR(Number(d.amount))}</td>
